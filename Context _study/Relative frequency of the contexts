...
for i, row in dftxt0.iteritems():
    ...
    for elem in list_sub_verbs:
        try:
            subj_dic[elem] = my_dictionary[elem]
...
def return_context(text, word, word2, window_size):
    ...
    for i in range(len(text)- window_size + 1):
        sublist = text[i:i+window_size]
        line = sublist[math.floor(window_size/2)].split() 
    
        line2 = sublist[math.floor(window_size/2)+1].split() 
        try:            
            if word == line[0].lower() and word2 == line2[0].lower():
                occurrence.append(sublist)
...
for i, row in dftxt0.iteritems():
    ...
    context=[]
    try:
        context = return_context(text1, "vaya", ",", 7)
        my_dic4["vaya"].append(len(context))
    except: 
        print('badline')
        my_dic4["vaya"].append(0)
    
    df_context = pd.DataFrame({'Context_vaya_comma': context})
   
    df_context.to_csv("_contextcomma_" + str(i) + ".csv", sep='\n', encoding='utf-8')


    
vayal_freqs = my_dic4["vaya"]
...
rel_freqs=[checkdiv(i ,j) for i, j in zip(vaya_freqs, vayal_freqs)]
