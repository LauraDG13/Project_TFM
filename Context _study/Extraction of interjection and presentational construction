...
for i, row in dftxt0.iteritems():
    ...
    context=[]
    try:
        context = return_context(text1, "vaya", "la", 7) 
    except: print('badline')
    
    df_context = pd.DataFrame({'Context': context})


    df_context.to_csv("_context_la" + str(i) + ".csv", sep='\n', encoding='utf-8')


#For obtaining the contexts in which "vaya" is used as an interjection and with the other presentational construction (vaya + NP; vaya + el),
#we change the selected context in line 89. 
